...
class MakeReservationDialog extends ComponentDialog {
    ...
    constructor(conservsationState, userState) {
        ...
        this.addDialog(new WaterfallDialog(WATERFALL_DIALOG, [
            ...
            this.inPowerStatusOn.bind(this),
            this.inPowerStatusOn2.bind(this),
            this.customerEquipment.bind(this),
    ...
    async inPowerStatusOn(step) {
        if (myapp.flag == true) {
            console.log("shutdown"+myapp.flag)
            myapp.flag2 = true;
            step.values.poe = step.result;
            ...
            myapp.flag2 = false;
            step.values.powerOff = step.result;
            if (step.values.powerOff == 'Power Outage') {
    ...
    async inPowerStatusOn2(step) {
        console.log("flag2: " + myapp.flag2)
        if (myapp.flag2 == true) {
            step.values.modem = step.result;
    ...
    async customerEquipment(step) {
        step.values.mux = step.result;
        var ce = MessageFactory.suggestedActions(['POE', 'Modem', 'MUX'], 'select customer equipment');
        await step.context.sendActivity(ce)
        return await step.prompt(TEXT_PROMPT, ce)
    }

    async forPoeorModem(step) {
        step.values.ce = step.result;
        if (step.values.ce == 'POE' || step.values.ce == 'Modem') {
            myapp.flag3 = 1;
    ...
    async routerPortLight(step) {
        ...
        if (myapp.flag3 == 1) {
            myapp.flag4 = 1;
            var rpl = MessageFactory.suggestedActions(['Admin light', 'Data light', 'Both', 'None'], 'select rout port light indication');
